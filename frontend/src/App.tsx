...
import { CVManagementStep } from "./components/CVManagementStep";
...
export default function App() {
  const [step, setStep] = useState<Step>("cv_management");
  ...
  const canAdvance = () => {
    if (step === "cv_management") return true;
        ...
        <div className="mx-auto max-w-7xl px-5">
          ...
          <div className="mx-auto mt-6 max-w-5xl">
            <AnimatePresence mode="wait">
              {step === "cv_management" && (
                <motion.div key="cv_management" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                  <CVManagementStep onViewCandidate={(candidateId, name) => setDetailCandidate({ candidateId, name })} />
                </motion.div>
              )}
