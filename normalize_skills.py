...
from app.normalize_sections.normalize_skills import normalize_casing, MANUAL_ALIASES
...
def main():
    ...
    if changed:
        print("  Casing changes found:")
        for raw, canonical in sorted(changed.items()):
            print(f"    {raw}  ->  {canonical}")

        print("\n  Copy these lines into CANONICAL_CASING in")
        print("  app/normalize_sections/normalize_skills.py to make this global and")
        print("  persistent (otherwise casing is only ever normalized within each CV's")
        print("  own list, not consistently across the whole database):\n")
        for raw, canonical in sorted(changed.items()):
            print(f'    "{raw.lower()}": "{canonical}",')
